...
function OrgUnitTimePeriod({ data_set_name , onSetDataSetId , onSetorgUnitId , onOTSelectSuccess , onSetTimePeriod}) {
    ...
    useEffect(() => {
      ...
      async function getDetails() {
        try {
            ...
            const res = await axios.get(`http://127.0.0.1:8000/dataset_org_units/${data_set_name}`, {
    ...
    const filteredOrgUnit = orgUnits.filter((orgUnit) => orgUnit.name.toLowerCase().includes(search.toLowerCase()));
    ...
    return (
        ...
        <header className="flex justify-end items-center  h-auto p-4">
            ...
            <main className="flex-1 flex flex-col justify-center items-center gap-3">
                ...
                <input type="text" className="border border-gray-400 bg-white px-3 rounded-full w-86.25 m-3" id="searchDataset" placeholder="Search OrgUnit"
                ...
                >
                    <option value="" disabled hidden>Choose an Org Unit...</option>

                    {filteredOrgUnit.map((orgUnits) =>

                        <option

                            key={orgUnits.id} value={orgUnits.id}>{orgUnits.name}</option>
                ...
                <input type="text" className="border border-gray-400 bg-white px-3 rounded-full w-86.25 m-3" placeholder="Enter Time Period"
                ...
                <label className="text-white text-xl" htmlFor="selectedDataset">Selected OrgUnit : </label>

                <div className="text-white text-xl" id="selectedDataset">{selectedOrgUnit}</div>

                </div>


                <button className="rounded-full px-6 py-2 bg-white text-black mt-2 hover:bg-gray-100 transition-colors"
                    onClick={() => { onOTSelectSuccess(); onSetorgUnitId(selectedOrgUnit)}}
                >Submit</button>
